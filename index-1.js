...
function buildTerminalHTML() {
    return `
    <div id="threadkeeper-overlay" style="position:fixed;inset:0;background:rgba(0,0,0,0.5);backdrop-filter:blur(3px);z-index:10000;opacity:0;pointer-events:none;transition:opacity 0.25s ease;">
        <div id="threadkeeper-terminal" style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%) scale(0.95);width:580px;max-width:94vw;max-height:84vh;display:flex;flex-direction:column;background:var(--tk-bg,#0a0e17);border:1px solid rgba(74,222,128,0.2);border-radius:10px;box-shadow:0 8px 48px rgba(0,0,0,0.6);overflow:hidden;font-family:'JetBrains Mono','Cascadia Code','Fira Code',monospace;z-index:10001;transition:transform 0.25s ease;">
            <!-- Header -->
            <div class="tk-header">
                <div class="tk-dots">
...
function openTerminal() {
    ...
    if (terminal) terminal.style.transform = 'translate(-50%, -50%) scale(1)';
    isTerminalOpen = true;
...
function closeTerminal() {
    ...
    if (terminal) terminal.style.transform = 'translate(-50%, -50%) scale(0.95)';
